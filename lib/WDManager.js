...
Manager.prototype.doStart = function(cb) {
    ...
    this.checkStatus(function(err, status) {
        ...
        me.proc = me.spawnCommand('java', args);
        console.log(('  Starting ' + driverConfig.selenium.name + ' with pid: ' + me.proc.pid + '. Press Ctrl+C to abort.').grey);
...
Manager.prototype.spawnCommand = function(command, args) {
    ...
    args = isWindows() ? ['/C'].concat(command, args) : args;
    return childProcess.spawn(command, args, { stdio: 'inherit' }); // use parent's stdios
};
