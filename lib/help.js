...
module.exports = function(program, opts) {
    ...
    program
        ...
        // .option('--parallel <num>', "number of parallel runs [1]", parseInt, 1)
        // .option('--parallel-strategy <name>', "'test' runs different tests in parallel. 'browser' runs the same test in mutiple browsers [test]", 'test')
        ...
        .option('--log <path>', "path to create a file log")
        .on('*', function(name) {
