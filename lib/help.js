...
module.exports = function(program, opts) {
    ...
    program
        .command('watch')
        .usage('[options] [path ...]')
        .description('watch is similar to run, except tests are only executed on file changes in the [target] path')
        .action(function(){
            var args = Array.prototype.slice.call(arguments, 0, -1);
            cmd.action = 'watch';
            cmd.args = args;
    ...
    program
        .command('generate')
        .usage('[options] [path]')
        .description('generate step definitions for a given feature')
        .option('--language <language>', 'The localized language of the source feature [English]', 'English')
